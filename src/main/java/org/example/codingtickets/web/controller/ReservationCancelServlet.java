    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        try {
            ...
            if (!(user instanceof Client)) {
                String msg = URLEncoder.encode("Seuls les clients peuvent annuler des réservations.", StandardCharsets.UTF_8);
                resp.sendRedirect(req.getContextPath() + "/events?error=" + msg);
                return;
            }
            ...
            resp.sendRedirect(req.getContextPath() + "/reservations/history?success=" + msg);

        } catch (AnnulationTardiveException e) {
            String msg = URLEncoder.encode(e.getMessage(), StandardCharsets.UTF_8);
            resp.sendRedirect(req.getContextPath() + "/reservations/history?error=" + msg);
        } catch (Exception e) {
            String msg = URLEncoder.encode("Erreur lors de l'annulation : " + e.getMessage(), StandardCharsets.UTF_8);
            resp.sendRedirect(req.getContextPath() + "/reservations/history?error=" + msg);
        }
    }
}
